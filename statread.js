...
async function extractKeyframes(video, timestamps) {
  let output = new Array(timestamps.length);
  for (let i = 0; i < timestamps.length; i++) {
    timestamps[i] = timestamps[i].split(':')[0] * 60 + timestamps[i].split(':')[1];  //Convert M:SS string to seconds
    let frame;
...
async function detectStats(image) {//Set up agents for text recognition via Tesseract.js
...
function bufferToStream(binary) {  //Converts the buffer to a stream so that it can be used as the source for the frame extraction
